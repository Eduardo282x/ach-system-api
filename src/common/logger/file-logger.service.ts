...
export class FileLoggerService {
    ...
    constructor() {
        ...
        this.combinedFile = path.join(this.logsDir, `combined-${this.currentDate}.log`);
    ...
    private formatLogEntry(entry: LogEntry): string {
        ...
        const status = entry.statusCode ? String(entry.statusCode).padEnd(5) : 'N/A  ';
        const time = entry.responseTime ? `${entry.responseTime}ms`.padStart(7) : '    N/A';
    ...
    private sanitizeBody(body: any): any {
        ...
        const sensitiveFields = ['password', 'token', 'secret', 'authorization', 'apiKey', 'api_key', 'access_token', 'refresh_token'];
